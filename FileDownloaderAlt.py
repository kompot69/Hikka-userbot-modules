...
@loader.tds
class FileDownloaderAltMod(loader.Module):
    """File Downloader (Uploader to Telegram) v2.1.A
    by @kompot_69 & @mirivan"""
    
    strings = {"name": "File Downloader Alt"}
        ...
        if match_arg or match_reply:
            ...
            if len(args) > 1:
                if args.index(url) == last_args_index:
                    return await message.client.send_message(message.peer_id,"1 "+usage)
                else:
                    text = args[1]
            ...
            if match_reply and len(match_reply) > 1:
                ...
                if index.isnumeric() and int(index) > 0:
                    ...
                    return await message.client.send_message(message.peer_id,"2 "+usage)
        else:
            return await message.client.send_message(message.peer_id,"3 "+usage)

        try:
            with requests.get(url.strip(), headers={'Accept-Encoding': None}, stream=True, timeout=(10, 5), verify=False) as r:
                if r.status_code == 404:
                    return await message.client.send_message(message.peer_id,"<b>Ошибка 404 при запросе на URL, указанный файл не найден.</b>")
                r.raise_for_status()
                filename = r.headers.get("Content-Disposition", "").split("filename=")[-1].split(';')[0].strip() or os.path.basename(urlparse(url).path)
                if not filename:
                    return await message.client.send_message(message.peer_id,"<b>Указаный URL не ведёт на файл.</b>")
                length = int(r.headers.get("Content-Length", 0))
                if not length:
                    return await message.client.send_message(message.peer_id,"<b>Не удалось получить размер файла.</b>")
                #me = await message.client.get_me()
                #is_premium =  getattr(me, "premium", False)
                #allowed_file_size = 4 if is_premium else 2
                allowed_file_size = 2
                if length > allowed_file_size * 1024 ** 3:
                    return await message.client.send_message(message.peer_id,f"<b>Файл не будет загружен, поскольку его размер (</b><code>{sizeof_fmt(length)}</code><b>) превышает допустимые для вас <code>{allowed_file_size}GB</code> для загрузки файлов.")
                length_h = sizeof_fmt(length)
                downloaded = 0
                message = await message.client.send_message(message.peer_id,f"<b>Загружаю файл</b>: <code>{filename}</code>")
            ...
            await message.client.send_message(message.peer_id,f"<b>Истекло время ожидания от сервера</b>: <code>{url}</code>")
        except Exception as e:
            traceback = re.sub("(`|\\|\\||\\*\\*|__|~~)", "", format_exc())
            await message.client.send_message(message.peer_id,f"<b>Произошла ошибка</b>:\n\n<code>{traceback}</code>")
        if filename:
            try:
